((Utils) => {
  const App = {
    htmlElements: {
      ...
      pokemonBody: document.querySelector("body"),
    ...
    handlers: {
      pokemonFinderFormOnSubmit: async (e) => {
        e.preventDefault();
        App.htmlElements.pokemonBody.style = "cursor: wait";
        ...
        try {
          ...
          App.htmlElements.pokemonFinderFilter.style = "cursor: default";
        } catch (error) {
          App.htmlElements.pokemonFinderFilter.style = "cursor: default";
          if (searchType == "pokemon") {
    ...
    templates: {
      render: async ({ searchType, response }) => {
        ...
        return renderMap[searchType]
          ? await renderMap[searchType](response)
      ...
      pokemonCard: async ({ id, name, weight, height, sprites, abilities, species }) => {
        ...
        return `<div class='title_card'>${name.toUpperCase()} (${id})</div>
                 <table width="100%">
                  <tr>
      ...
      abilityCard: async ({ id, names, pokemon }) => {
        const pokemonList = pokemon.map(
          ({ pokemon, is_hidden }) =>
            `<li>${pokemon.name}${is_hidden ? App.htmlElements.svgpokemonhidden : ""
            }</li>`
        );
        let nameAbility = Utils.nameAbility({ names });
        return `<span class='span_title_ability'>${nameAbility} (${id})</span><ul>${pokemonList.slice(0, 5).join("")}</ul>`;
      },
      ...
      pokemonEvolutions: async ({ species }) => {
        const evoluciones = await Utils.getEvoluciones({ species });
